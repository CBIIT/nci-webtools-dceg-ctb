                ...
                if request.user.is_authenticated:
                    ...
                    if not (request.user.is_superuser and re.match('/admin/?.*', request.path, re.I)):
                        if not EmailAddress.objects.filter(
                            ...
                            message = "We have sent an e-mail to you for verification. Please <a href=\'#logout-modal\' data-bs-toggle=\'modal\' data-bs-target=\'#logout-modal\'>log out</a> and follow the link provided to finalize the signup process. If you have any questions, please email us at <a href='mailto:ctbWebAdmin@mail.nih.gov'>ctbWebAdmin@mail.nih.gov</a>"
                        elif not request.user.groups.filter(
                                reduce(lambda q, g: q | Q(name__icontains=g), settings.RESTRICTED_ACCESS_GROUPS,
                                       Q())).exists():
                            message = "Your account application will need to be evaluated before an account can be approved. You will get an email about approval. If you have any questions, please email us at <a href='mailto:ctbWebAdmin@mail.nih.gov'>ctbWebAdmin@mail.nih.gov</a>"
                    if message:
